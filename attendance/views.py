...
def home(request):
    return render(request, 'home.html', {'name':'Tal crafts'})

def add(request):

    name = str(request.POST['company_name'])
    branch = str(request.POST['Branch_name'])
    Employee_name= str(request.POST['Branch_name'])

    name = name
    branch = branch
    
    return render(request, 'result.html',{'employee_name':Employee_name})
